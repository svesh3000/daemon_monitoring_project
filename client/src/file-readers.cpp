...
ClientConfig::ClientConfig(const std::string & filepath):
  ClientConfig()
{
  loadFile(filepath);
}

void ClientConfig::loadFile(const std::string & filepath)
{
...
const std::string & ClientConfig::getMetricsFilePath() const noexcept
{
  return metrics_filepath_;
}

const std::map< std::string, ServerInfo > & ClientConfig::getServerInfoMap() const noexcept
{
  return servers_;
}
...
const ServerInfo & ClientConfig::getServerInfo(const std::string & server_name) const
{
...
MetricsFile MetricsPackage::loadFile(const std::string & filepath)
{
  ...
  if (!raw_package)
  {
    throw std::invalid_argument("cannot open metrics file");
  }
